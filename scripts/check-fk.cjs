...
const sql = `SELECT conname FROM pg_constraint WHERE conrelid = 'public.users'::regclass AND confrelid = 'public.users'::regclass;`;
...
https.request(opts, r => { let d = ''; r.on('data', c => d += c); r.on('end', () => console.log(d)); }).end(body);
