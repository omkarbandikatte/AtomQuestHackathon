...
-- Check goal_sheets policies
SELECT polname, polrelid::regclass::text AS table_name, polcmd, pg_get_expr(polqual, polrelid) AS using_expr, pg_get_expr(polwithcheck, polrelid) AS with_check
FROM pg_policy
WHERE polrelid IN ('public.goal_sheets'::regclass, 'public.goals'::regclass)
